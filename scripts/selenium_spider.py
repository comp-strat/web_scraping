...
class SchoolSpider(RedisSpider):
    name = "selenium"
    # start_urls = ["https://www.richland2.org/charterhigh"]
    ...
    def parse(self, response):
        ...
        self._logger.debug("Populated most of item")
        ...
        if cur_depth >= response.meta['maxdepth']:
            ...
            try:
                ...
                item["body"] = link.text
                for l in new_links:
                    item["links"].append({"url": l.hrefAttribute, "text": l.text, })
                    request = scrapy.Request(l.hrefAttribute, callback=self.parse)
                    request.meta["link"] = l
                    request.meta['priority'] = response.meta['priority'] - 10
                    ...
                    self._logger.debug("Trying to follow link '{}'".format(request.url))
                    yield request
            except LinkException:
